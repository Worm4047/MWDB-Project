...
def getLatentSemantic(k, decompType, dataMatrix, modelType, label, imageDirName, imagePaths):
    ...
    latent_semantic = latentSemanticsSaver.getSemanticsFromFolder(lsPath)
    if latent_semantic is None:
        ...
        # u, s, v = PCA(dataMatrix, k).getDecomposition()
            ...
            print("Check later")
            return None
        latentSemanticsSaver.saveSemantics(os.path.basename(imageDirName), modelType, label, decompType, k, latent_semantic[0], latent_semantic[1], imagePaths=imagePaths)
